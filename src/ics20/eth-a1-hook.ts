 ...
 */
export const predictProxy = (src: string) => Effect.fn("predictProxy")(function *(options: { path: bigint;
  channel: ChannelId;
  ...
  const salt = yield* pipe(
    Effect.try(() => encodeAbiParameters(
      abi,
      [
        options.path,
        options.channel,
  ...
  const sha256 = Effect.fn((data: any) => Effect.tryPromise(() => globalThis.crypto.subtle.digest(
    "SHA-256",
    data
  )));
  ...
  const address = yield* pipe(
    ...
    Effect.flatMap(Schema.decode(Ucs05.Bech32FromCanonicalBytesWithPrefix("osmo")))
  );

  return Ucs05.CosmosDisplay.make({ address });
});

export const makeEthToAtoneTransaction = async (src: string, _dest: string, sender: string, rcpt: string, amount: bigint, baseToken: string, quoteToken: string, solver_metadata: string) => {
  return Effect.gen(function *() {
    ...
    const sourceChain = src === "ethereum"
    ...
    const request = ZkgmClientRequest.make({
      source: sourceChain,
      destination: osmosisChain,
      channelId: src === "ethereum"
        ? ETH_SOURCE_CHANNEL_ID
        : BASE_SOURCE_CHANNEL_ID,
      ucs03Address: src === "ethereum"
        ? UCS03_ETH_EVM.address
        : UCS03_BASE_EVM.address,
      instruction: batch
    });
