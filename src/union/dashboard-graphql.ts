...
const ATONE_IBC_HEX = "0x" + hexEncode("ibc/BC26A7A805ECD6822719472BCB7842A48EF09DF206182F8F259B2593EB5D23FB");
const PHOTON_IBC_HEX = "0x" + hexEncode("ibc/D6E02C5AE8A37FC2E3AB1FC8AC168878ADB870549383DFFEA9FD020C234520A7");
...
export function resolveTokenSymbol (transfer: RawTransfer): string {
  ...
  // Check IBC denom hex
  if (base === ATONE_IBC_HEX.toLowerCase() || quote === ATONE_IBC_HEX.toLowerCase()) return "ATONE";
  if (base === PHOTON_IBC_HEX.toLowerCase() || quote === PHOTON_IBC_HEX.toLowerCase()) return "PHOTON";
...
export async function fetchTransferHistory (filters: TransferFilters = {}): Promise<Transfer[]> {
  const variables: Record<string, unknown> = {
    p_limit: filters.limit ?? 20,
    p_comparison: filters.comparison ?? "lt"
  };
  if (filters.sourceChainId) variables.p_source_universal_chain_id = filters.sourceChainId;
...
export async function fetchTransferStats (daysBack = 30, sourceChainId?: string, destinationChainId?: string): Promise<TransferStats[]> {
