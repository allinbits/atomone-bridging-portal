...
const LOCKS_DIR = join(process.cwd(), ".tmp", "e2e-locks");
...
export const withFileLock = async <T>(name: string, work: () => Promise<T>): Promise<T> => {
  ...
  while (true) {
    try {
      ...
      if (await isStale(lockPath, DEFAULT_STALE_TIMEOUT_MS)) {
      ...
      if (Date.now() - startedAt > DEFAULT_WAIT_TIMEOUT_MS) {
        ...
        try {
          ownerDetails = await readFile(ownerPath, "utf8");
        } catch {
          // Ignore owner read failures and keep timeout error actionable.
        }
        throw new Error(
          `Timed out waiting for lock "${name}" after ${DEFAULT_WAIT_TIMEOUT_MS}ms. ` +
