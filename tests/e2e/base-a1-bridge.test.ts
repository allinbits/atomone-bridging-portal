...
describe(
  "Base → AtomOne Bridge E2E",
  () => {
    it.skipIf(!hasEnv)(
      "bridges tokens from Base to AtomOne via Osmosis + Union ZKGM",
      async () => {
        const atomoneWallet = await DirectSecp256k1HdWallet.fromMnemonic(
          MNEMONIC!,
          {
            prefix: "atone"
          }
        ...
        expect(
          ethBalance,
          "Insufficient ETH balance on Base for gas"
        ).toBeGreaterThan(0n);

        const route = routes.find((r) => r.src === "Base" && r.dest === "AtomOne" && r.denom === DENOM);
        expect(
          route,
          `No route found for Base → AtomOne ${DENOM}`
        ).toBeDefined();
